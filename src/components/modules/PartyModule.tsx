      ...
      {tab === "consignment" && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={() => { resetCForm(); setShowCForm(!showCForm); }} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-gold text-primary-foreground text-sm font-medium hover:opacity-90 transition-opacity">
          ...
          {showCForm && (
            <div className="rounded-xl border border-primary/20 p-6 bg-gradient-card shadow-card animate-slide-in">
              ...
              <div className="flex gap-3 mt-4">
                ...
                <button type="button" onClick={resetCForm} className="px-4 py-2 rounded-lg border border-border text-muted-foreground text-sm">{t("cancel")}</button>
              </div>
            </div>
          )}
                      ...
                      <div className="flex items-center justify-between p-3 hover:bg-secondary/30 cursor-pointer" onClick={() => setExpandedC(isExpanded ? null : c.id)}>
                        ...
                        <div className="flex items-center gap-3 shrink-0">
                          ...
                          <div className="flex items-center gap-1">
                            {can_edit && <button type="button" aria-label="Edit consignment" onClick={e => { e.stopPropagation(); handleEditConsignment(c); }} className="p-1 rounded hover:bg-secondary"><Pencil className="w-3.5 h-3.5 text-muted-foreground" /></button>}
                            {can_delete && <button type="button" aria-label="Delete consignment" onClick={e => { e.stopPropagation(); handleDeleteConsignment(c.id); }} className="p-1 rounded hover:bg-destructive/10"><Trash2 className="w-3.5 h-3.5 text-destructive/70" /></button>}
                          </div>
                        </div>
                      </div>
                            ...
                            <div className="space-y-1">
                              {cReturns.map((r, idx) => (
                                <div key={r.id} className="flex items-center justify-between px-3 py-1.5 rounded bg-secondary/30 text-xs">
                                  ...
                                  {can_delete && <button type="button" aria-label="Delete return" onClick={() => handleDeleteReturn(r.id, c.id)} className="p-0.5 rounded hover:bg-destructive/10"><Trash2 className="w-3 h-3 text-destructive/60" /></button>}
                                </div>
                              ))}
                            </div>
                          ...
                          {showReturnForm === c.id && (
                            <div className="mt-3 p-3 rounded-lg border border-primary/20 bg-background space-y-3">
                              ...
                              <div className="flex gap-2">
                                ...
                                <button type="button" onClick={() => setShowReturnForm(null)} className="px-3 py-1.5 rounded-lg border border-border text-muted-foreground text-xs">{t("cancel")}</button>
                              </div>
                            </div>
                          )}
                        </div>
      ...
      {tab === "settlement" && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={() => { resetSForm(); setShowSForm(!showSForm); }} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-gold text-primary-foreground text-sm font-medium hover:opacity-90 transition-opacity">
          ...
          {showSForm && (
            <div className="rounded-xl border border-primary/20 p-6 bg-gradient-card shadow-card animate-slide-in">
              ...
              <div className="flex gap-3">
                <button type="button" onClick={handleSaveSettlement} className="px-4 py-2 rounded-lg bg-gradient-gold text-primary-foreground text-sm font-medium">{t("save")}</button>
                <button type="button" onClick={resetSForm} className="px-4 py-2 rounded-lg border border-border text-muted-foreground text-sm">{t("cancel")}</button>
              </div>
            </div>
          )}
