...
const SettingsModule = () => {
  ...
  const togglePerm = async (permId: string, field: "can_edit" | "can_delete", val: boolean) => {
    const { error } = await supabase.from("role_permissions").update({ [field]: val } as any).eq("id", permId);
  ...
  const getRoleModules = (r: string) => getModulesForRole(r as any);
  ...
  const handleCreateUser = async () => {
    ...
    try {
      ...
      toast.error(err.message || t("userCreateFailed"));
    }
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="rounded-xl border border-border p-6 bg-gradient-card shadow-card">
        ...
        <div className="flex gap-3">
          <button onClick={() => setLang("bn")} className={`px-4 py-2.5 rounded-lg text-sm font-medium transition-all ${lang === "bn" ? "bg-gradient-gold text-primary-foreground shadow-gold" : "border border-border text-muted-foreground hover:bg-secondary"}`}>
            🇧🇩 বাংলা
          </button>
          <button onClick={() => setLang("en")} className={`px-4 py-2.5 rounded-lg text-sm font-medium transition-all ${lang === "en" ? "bg-gradient-gold text-primary-foreground shadow-gold" : "border border-border text-muted-foreground hover:bg-secondary"}`}>
            🇬🇧 English
          </button>
        </div>
      </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => fileInputRef.current?.click()} disabled={uploadingLogo} className="px-4 py-2 rounded-lg bg-gradient-gold text-primary-foreground text-sm font-medium hover:opacity-90 transition-opacity disabled:opacity-50">
                {uploadingLogo ? t("loading") : t("uploadLogo")}
              </button>
              {logoUrl && (
                <button onClick={handleLogoDelete} className="px-3 py-2 rounded-lg border border-destructive/30 text-destructive text-sm font-medium hover:bg-destructive/10 transition-colors flex items-center gap-1">
      ...
      <div className="rounded-xl border border-border p-6 bg-gradient-card shadow-card">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => sigInputRef.current?.click()} disabled={uploadingSig} className="px-4 py-2 rounded-lg bg-gradient-gold text-primary-foreground text-sm font-medium hover:opacity-90 transition-opacity disabled:opacity-50">
                {uploadingSig ? t("loading") : lang === "bn" ? "সিগনেচার আপলোড" : "Upload Signature"}
              </button>
              {signatureUrl && (
                <button onClick={handleSignatureDelete} className="px-3 py-2 rounded-lg border border-destructive/30 text-destructive text-sm font-medium hover:bg-destructive/10 transition-colors flex items-center gap-1">
                  <Trash2 className="w-3.5 h-3.5" />
                  {lang === "bn" ? "মুছুন" : "Delete"}
                </button>
              )}
      ...
      <div className="rounded-xl border border-border p-6 bg-gradient-card shadow-card">
        ...
        <div className="space-y-3">
          ...
          <button onClick={handleSaveCompany} disabled={savingCompany} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-gold text-primary-foreground text-sm font-medium hover:opacity-90 transition-opacity disabled:opacity-50">
      ...
      {isAdmin && (
        <div className="rounded-xl border border-border p-6 bg-gradient-card shadow-card">
          ...
          <div className="space-y-3">
            ...
            <button onClick={handleCreateUser} disabled={creatingUser || !newEmail || !newPassword} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-gold text-primary-foreground text-sm font-medium hover:opacity-90 transition-opacity disabled:opacity-50">
                          ...
                          {rolePerms.map(p => (
                            <tr key={p.id} className="border-t border-border/50">
                              ...
                              <td className="px-3 py-2 text-center">
                                <button aria-label="Toggle edit" onClick={() => togglePerm(p.id, "can_edit", !p.can_edit)}
                              ...
                              <td className="px-3 py-2 text-center">
                                <button aria-label="Toggle delete" onClick={() => togglePerm(p.id, "can_delete", !p.can_delete)}
                              ...
                              <td className="px-3 py-2 text-center">
                                <button aria-label="Toggle print" onClick={() => togglePerm(p.id, "can_print" as any, !(p as any).can_print)}
                              ...
                              <td className="px-3 py-2 text-center">
                                <button aria-label="Toggle download" onClick={() => togglePerm(p.id, "can_download" as any, !(p as any).can_download)}
