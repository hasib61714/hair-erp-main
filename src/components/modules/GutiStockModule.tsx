...
const GutiStockModule = () => {
  ...
  const fetchData = async () => {
    ...
    setData((rows || []).map((r: any) => ({ ...r, grade_details: r.grade_details || [] })));
    setFactories(f || []);
    setLoading(false);
  };
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      notes: notes || null,
      grade_details: gradeRows.filter(r => r.kg > 0) as any,
    };
  ...
  const handleEdit = (e: GutiEntry) => {
    ...
    const gd = (e.grade_details && (e.grade_details as any).length > 0)
      ? (e.grade_details as GradeDetail[])
            ...
            <div className="space-y-2">
              {gradeRows.map((row, idx) => (
                <div key={idx} className="grid grid-cols-[1fr_1fr_1fr_auto_auto] gap-2 items-center">
                  ...
                  <button type="button" onClick={() => removeGradeRow(idx)} className="p-1 rounded hover:bg-destructive/10"><XCircle className="w-4 h-4 text-destructive/70" /></button>
                </div>
              ))}
            </div>
