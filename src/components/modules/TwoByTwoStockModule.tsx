...
const TwoByTwoStockModule = () => {
  ...
  const fetchData = async () => {
    ...
    setData((rows ?? []).map(r => ({ ...r, grade_details: Array.isArray(r.grade_details) ? (r.grade_details as unknown as GradeRow[]) : [], carton_no: r.carton_no ?? "", factory_name: r.factory_name ?? "", guti_type: r.guti_type ?? "", guti_cost_per_kg: Number(r.guti_cost_per_kg) || 0 })));
    setLoading(false);
  };
  useEffect(() => { fetchData(); }, []);
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      grade_details: gradeRows as unknown as Json, total_input_kg: input, total_output_kg: totalOutputCalc,
        ...
        <div className="rounded-xl border border-primary/20 p-6 bg-gradient-card shadow-card animate-slide-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
            ...
            <div>
              ...
              <input aria-label="Owner name" value={ownerName} onChange={e => setOwnerName(e.target.value)} className="w-full h-9 rounded-lg border border-border bg-secondary/50 px-3 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-primary" />
            </div>
            <div>
            ...
            <div>
              ...
              <input aria-label="Total input KG" type="number" value={totalInputKg} onChange={e => setTotalInputKg(e.target.value)} className="w-full h-9 rounded-lg border border-border bg-secondary/50 px-3 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-primary" />
            </div>
          </div>
